    ...
    e.fn.extend({
        showbizpro: function(n) {
            ...
            return this.each(function() {
                function i() {
                    f.data("timer", setInterval(function() {
                        if (f.width() > 0) {
                            if (f.data("scrollOrientation") == "right") {
                                var t = e(f.find(".showbiz").data("left"));
                                if (!f.hasClass("hovered")) u(f, t, true)
                            } else {
                                var t = e(f.find(".showbiz").data("right"));
                                if (!f.hasClass("hovered")) a(f, t, true)
                            }
                        }
                    }, n.delay))
                }
                ...
                function o() {
                    clearInterval(f.data("timer"))
                }
                var f = e(this);
                if (n.forceFullWidth == true) {
                    var h = f.offset().left;
                    var p = f.css("marginBottom");
                    var d = f.css("marginTop");
                    if (p == t) p = 0;
                    if (d == t) d = 0;
                    f.wrap('<div style="position:relative;width:100%;height:auto;margin-top:' + d + ";margin-bottom:" + p + '" class="forcefullwidth_wrapper_tp_banner"></div>');
                ...
                f.data("eoffset", n.entrySizeOffset);
                f.data("croffset", n.containerOffsetRight);
                f.data("hboffset", n.heightOffsetBottom);
                f.data("ease", n.easing);
                if (n.carousel == "on") f.data("carousel", 1);
                else f.data("carousel", 0);
                f.data("ytmarkup", n.ytMarkup);
                f.data("vimeomarkup", n.vimeoMarkup);
                f.data("vea", n.visibleElementsArray);
                f.data("coo", n.closeOtherOverlays);
                f.data("allentry", n.allEntryAtOnce);
                f.data("mediaMaxHeight", n.mediaMaxHeight);
                f.data("das", n.dragAndScroll);
                f.data("rewindfromend", n.rewindFromEnd);
                f.data("forceFullWidth", n.forceFullWidth);
                f.data("scrollOrientation", n.scrollOrientation);
                f.data("currentoffset", 0);
                n.speed = parseInt(n.speed, 0);
                n.delay = parseInt(n.delay, 0);
                f.data("speedy", n.speed);
                f.data("ie", !e.support.opacity);
                f.data("ie9", document.documentMode == 9);
                var v = e.fn.jquery.split("."),
                    m = parseFloat(v[0]),
                    g = parseFloat(v[1]),
                    y = parseFloat(v[2] || "0");
                if (m > 1) f.data("ie", false);
                if (n.carousel == "on") {
                    if (f.find("ul").first().find(">li").length < 17) {
                        f.find("ul").first().find(">li").each(function(t) {
                            e(this).clone(true).appendTo(f.find("ul").first())
                        })
                    }
                    if (f.find("ul").first().find(">li").length < 17) {
                        f.find("ul").first().find(">li").each(function(t) {
                ...
                var b = f.find(".showbiz");
                b.attr("id", "sbiz" + Math.round(Math.random() * 1e4));
                var w;
                b.find("img").on("dragstart", function(e) {
                    e.preventDefault()
                });
                if (n.dragAndScroll == "off") {
                    var E = b.find(".overflowholder ul");

                    function S(e) {
                        var n = E.find("li").first();
                        var r = E.data("newpos");
                        if (r == t) {
                            r = E.position().left;
                            E.data("newpos", r)
                    ...
                    x.on("dragstart", S);
                    x.on("release", S);
                    E.addClass("showbiz-drag-mouse")
                }
                var T = Math.round(Math.random() * 1e5);
                if (b.data("left") == t) b.data("left", "sb_left_" + T);
                if (b.data("right") == t) b.data("right", "sb_right_" + T);
                if (b.data("play") == t) b.data("play", "sb_play_" + T);
                var N = e(b.data("left"));
                var C = e(b.data("right"));
                var k = e(b.data("play"));
                if (N == t || N.length == 0) e("body").append('<a style="display:none" id="' + b.data("left") + '" class="sb-navigation-left"><i class="sb-icon-left-open"></i></a>');
                if (C == t || C.length == 0) e("body").append('<a style="display:none" id="' + b.data("right") + '" class="sb-navigation-right"><i class="sb-icon-right-open"></i></a>');
                if (k == t || k.length == 0) e("body").append('<a style="display:none" id="' + b.data("play") + '" class="sb-navigation-play"><i class="sb-icon-play sb-playbutton"></i><i class="sb-icon-pause sb-pausebutton"></i></a>');
                l(f, b);
                if (n.autoPlay != "on") {
                    e(f.find(".showbiz").data("play")).remove()
                } else {
                    i();
                    var k = e(f.find(".showbiz").data("play"));
                    k.find(".sb-playbutton").addClass("sb-hidden");
                    f.hover(function() {
                        f.addClass("hovered")
                    }, function() {
                    ...
                    k.click(function() {
                        if (k.hasClass("paused")) {
                            i();
                            k.removeClass("paused");
                            k.find(".sb-pausebutton").removeClass("sb-hidden");
                            k.find(".sb-playbutton").addClass("sb-hidden")
                        } else {
                            o();
                            k.addClass("paused");
                            k.find(".sb-pausebutton").addClass("sb-hidden");
                            k.find(".sb-playbutton").removeClass("sb-hidden")
                        }
                    })
                }
                s(f, b);
                try {
                    f.find(".mediaholder_innerwrap").each(function() {
                        var t = e(this);
