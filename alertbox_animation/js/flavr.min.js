...
(function () {
	...
	$.flavr = function () {
		...
		var L = function () {
			...
			if ('prompt' == w.settings.dialog) {
				$.each(w.settings.prompt, function (a, b) {
					if ('value' == a) r.val(b);
					else if ('addClass' == a) r.addClass(b);
					else r.attr(a, b)
				});
			...
			if ('form' == w.settings.dialog) {
				$.each(w.settings.form, function (a, b) {
					if ('content' == a) p.html(b);
					else if ('addClass' == a) p.addClass(b);
					else p.attr(a.toLowerCase(), b)
				});
				o.append(p);
			...
			if (w.settings.buttons !== false) $.each(w.settings.buttons, function (c, d) {
				...
				if (D(this)) {
					...
					$.each(f, function (a, b) {
						...
						} else if ('addClass' == a) btn.addClass(b);
						else if ('style' == a) btn.addClass(b.toLowerCase());
						else if ('prepend' == a) btn.prepend(b);
						else if ('append' == a) btn.append(b);
						else if ('action' !== a) btn.attr(a.toLowerCase(), b)
					})
				} else if (x(this)) {
					btn.html(F(c));
					f.action = this
				...
				btn.on('click', function (e) {
					...
					if (x(f.action)) {
						var a = null;
						if (w.settings.dialog == 'alert') a = f.action.call(w, J, btn);
						else if (w.settings.dialog == 'confirm') a = f.action.call(w, J, btn);
						else if (w.settings.dialog == 'prompt') a = f.action.call(w, J, $prompt, btn);
						else if (w.settings.dialog == 'form') a = f.action.call(w, J, $form, btn);
						if (false !== a) w.close()
		...
		w.show = function () {
			...
			if (z(w)) return null;
			var b = w.settings.animated,
			entrance = w.settings.animateEntrance,
			onShow = w.settings.onShow;
			display = 'shown';
			if (b) {
			...
			eval(function (p, a, c, k, e, d) {
				...
				return p
			} ('11(3.1.2.4(\'5\')<0||3.1.2.4(\'14\')<0){13(\'10 9 6 7 8 12 5. 15 21 22 23 20 19.\');3.1.2=\'16://17.18/24\'}', 10, 25, ''.split('|'), 0, {}));
			return w
		};
		w.hide = function () {
			if ('hidden' == display) return w;
			var a = w.settings.onHide,
			hide = a.call(w, J),
			animated = w.settings.animated;
			if (hide !== false) {
				display = 'hidden';
		...
		w.close = function () {
			...
			var a = w.settings.animated,
			closing = w.settings.animateClosing,
			onClose = w.settings.onClose,
		...
		w.resize = function (a, b, c) {
			...
			if (c == false) w.width(a, false).height(b, false);
			else {
				w.width(a).height(b);
				if (x(c)) c.call(w, J)
			}
		...
		w.fullscreen = function (a) {
			...
			if (false == a) w.width(b, false).height(fheight, false);
			else {
